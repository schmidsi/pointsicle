    ...
    <>
      ...
      <div className="hidden max-w-screen-xl w-full mt-16 mx-auto h-full flex-1 flex-col space-y-8 p-8 md:flex">
        <div className="flex items-center justify-between space-y-2">
          <div>
            <h2 className="text-4xl font-bold tracking-tight">{data.data.pointsicleCampaigns[0].displayName} Campaign</h2>
            <p className="text-xl text-muted-foreground">
              {data.data.pointsicleCampaigns[0].description}
            </p>
          </div>
        </div>
        ...
        <section className="w-full mb-10">
          ...
          <div className="w-full rounded-2xl bg-gray-50 border border-gray-200 overflow-hidden">
            ...
            {
              data.data.quests.map((quest: any, index: number) => {
